...
const rutaProd = path.resolve(__dirname, "../data/productos.json");

class ApiProds {
  async obtenerDatos() {
    
    const recData = JSON.parse(await fs.readFile(rutaProd, "utf-8"));
    const dataMock = createRandomProducts();
    
    let data = recData.concat(dataMock);
  ...
  async newElement(data: any) {
    ...
    if (
      ...
      throw new Error("Los campos estan incompletos");
